  ...
  context "#init_gem_files" do
    context "in case of empty file name" do
      it "should raise RocketApi::InitFilesError" do
        expect{ RocketApi::RocketCommands.init_gem_files({}) }.to raise_error(RocketApi::InitFilesError, "Init action fail: Project name is empty")
      end
    end
  end
  ...
  context "#init_gem_dir" do
    context "in case of not empty dir" do
      it "should raise RocketApi::CreateDirError" do
        expect{ RocketApi::RocketCommands.init_gem_dir({}) }.to raise_error(RocketApi::CreateDirError, "Create failed: another one gem exist")
      end
    end
  end
end